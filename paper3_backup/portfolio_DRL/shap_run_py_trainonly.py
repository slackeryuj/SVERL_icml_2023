...
if state == 'train_data_generate':
    ...
    for start in range(0, 200, window_size):
        end = start + window_size
        train_data = df_shap.iloc[start:end]
        env = make_vec_env(lambda: PortfolioTradingEnv(train_data, df_returns, initial_balance=1000, etfs=etfs), n_envs=1)
        ...
        # Generate out-of-sample weights for next 78 periods
        test_data = df_shap.iloc[end:end + window_size]
        ...
        start_dates = df_shap.index.get_level_values(0)[end:end + 78]
        end_dates = df_shap.index.get_level_values(1)[end:end + 78]
        for i in range(len(avg_weights)):
            out_of_sample_weights.append([start_dates[i], end_dates[i], *avg_weights[i]])

    # Save the results
    out_of_sample_weights = np.squeeze(np.array(out_of_sample_weights))
    columns = ['Start Date', 'End Date'] + etfs
