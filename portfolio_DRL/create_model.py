...
class StockDataset_ETF(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(X, dtype=torch.float32), torch.from_numpy(y_tensor)
...
class PortfolioTradingEnv_old(gym.Env):
...
class PortfolioTradingEnv(gym.Env):
    def __init__(self, data, returns, initial_balance=1000, etfs=None, use_cash=True, lambda_penalty=0.5):
        super(PortfolioTradingEnv, self).__init__()
        self.data = data
        self.returns = returns
        self.initial_balance = initial_balance
        self.use_cash = use_cash
        self.lambda_penalty = lambda_penalty  # Weight for drawdown penalty
        self.current_step = 0
        self.etfs = etfs if etfs else []
        self.num_etfs = len(self.etfs)

        # Define action and observation spaces
        self.action_dim = self.num_etfs + 1 if self.use_cash else self.num_etfs
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(data.shape[1],), dtype=np.float32)
        self.action_space = spaces.Box(low=0, high=1.0, shape=(self.action_dim,), dtype=np.float32)

        # Initialize portfolio
        self.portfolio_value = self.initial_balance
        self.weights = np.ones(self.action_dim) / self.action_dim
    ...
    def reset(self):
        ...
        if self.use_cash:
            self.cash_value = self.portfolio_value / (self.num_etfs + 1)
            self.etf_value = self.portfolio_value - self.cash_value
            etf_weights = np.ones(self.num_etfs) / self.num_etfs
            self.weights = np.append(etf_weights * (self.etf_value / self.portfolio_value),
                                     self.cash_value / self.portfolio_value)
        else:
            self.weights = np.ones(self.num_etfs) / self.num_etfs
    ...
    def step(self, action):
        ...
        if self.use_cash:
            # Extract cash and ETF weights
            etf_weights = action[:-1]
            cash_weight = action[-1]

            # Convert to dollar allocation
            self.cash_value = self.portfolio_value * cash_weight
            etf_value = self.portfolio_value - self.cash_value
            etf_dollar_alloc = etf_value * etf_weights

            for daily_return in period_returns:
                etf_dollar_alloc *= (1 + daily_return)
                etf_value = np.sum(etf_dollar_alloc)
                self.portfolio_value = etf_value + self.cash_value
                daily_returns.append((self.portfolio_value / portfolio_values[-1]) - 1)
                portfolio_values.append(self.portfolio_value)

            # Recalculate weights
            etf_weights_updated = etf_dollar_alloc / self.portfolio_value
            cash_weight_updated = self.cash_value / self.portfolio_value
            self.weights = np.append(etf_weights_updated, cash_weight_updated)
            ...
            for daily_return in period_returns:
                portfolio_return = np.dot(daily_return, self.weights)
                self.portfolio_value *= (1 + portfolio_return)
        ...
        std_return = np.std(daily_returns) if np.std(daily_returns) > 0 else 1e-6  # Avoid division by zero
        sharpe_ratio = mean_return / std_return

        # Compute Max Drawdown
        max_portfolio_value = np.maximum.accumulate(portfolio_values)
        drawdowns = (portfolio_values - max_portfolio_value) / max_portfolio_value
        max_drawdown = np.min(drawdowns) if len(drawdowns) > 0 else 0

        # Final Reward: Sharpe Ratio - λ × Max Drawdown
        reward = sharpe_ratio - self.lambda_penalty * abs(max_drawdown)
    ...
    def render(self):
        weight_str = ', '.join([f"{w:.2%}" for w in self.weights])
        print(f"Step: {self.current_step}, Portfolio Value: {self.portfolio_value:.2f}, Weights: [{weight_str}]")
