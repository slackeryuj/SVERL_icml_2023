...
# NOTE: We avoid importing scikit-learn's StandardScaler here to prevent
# reliance on SciPy, which can cause DLL load errors on some systems.
# Instead, we implement a minimal standard scaler class that computes
# per-feature means and standard deviations via pandas/numpy.  This class
# exposes the same interface (fit, transform) as sklearn's StandardScaler
# where needed for our use case.
...
class _SimpleStandardScaler:
    ...
    def fit(self, X: pd.DataFrame) -> '_SimpleStandardScaler':
        """Compute the mean and standard deviation for each column."""
        self.mean_ = X.mean(axis=0)
        # Use ddof=0 for population standard deviation; replace zeros to avoid division by zero
        self.scale_ = X.std(axis=0, ddof=0).replace(0, 1)
        return self

    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        """Standardise columns using the previously computed statistics."""
...
def scale_features(train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, list[str], _SimpleStandardScaler]:
    """
    ...
    feature_cols = [c for c in train_df.columns
                    if c not in ['Date'] and not c.startswith('Actual_Return')]
    scaler = _SimpleStandardScaler().fit(train_df[feature_cols])
...
def train_and_evaluate(
    ...
    # Determine whether to reuse a pretrained model.  When a valid path is
    # provided and the file exists, training is skipped and the existing
    # model is loaded directly for evaluation.
    use_pretrained = False
    if pretrained_model_path is not None and os.path.exists(pretrained_model_path):
        use_pretrained = True

    # Determine the compute device once upfront
    if device == 'auto':
        device_used = 'cuda' if torch.cuda.is_available() else 'cpu'
    else:
        device_used = device

    # Always create an evaluation environment.  This environment is used for
    # periodic evaluation during training and for loading a pretrained model when
    # training is skipped.
    ...
    # Set up the log directory and training history path.  A distinct
    # directory is created for each seed to avoid overwriting artefacts.
    if seed is not None:
    ...
    # Placeholder variables for outputs; these will be set in either branch
    final_model_path = None
    best_model_path = None

    # ---------------------------------------------------------------------
    # Training or loading of model
    # ---------------------------------------------------------------------

    # Determine a logs directory for saving models and history.  If a seed is
    # provided, the directory name is suffixed with the seed to avoid
    # collisions when running multiple experiments.
    if seed is not None:
        logs_dir = Path(f'logs_seed{run_seed}')
    else:
        logs_dir = Path('logs')
    ...
    # Paths for training history and best model
    training_history_path = logs_dir / 'training_history.csv'
    best_model_path = logs_dir / 'best_model.zip'

    # Variables that will be set in the relevant branch below
    best_model: PPO | None = None
    final_model_path: str | None = None

    if not use_pretrained:
        # -----------------------------------------------------------------
        # Training branch: create training environment, train the model and
        # evaluate
        # -----------------------------------------------------------------
        # Vectorised training environment using multiple processes.  Using
        # SubprocVecEnv speeds up experience collection by running multiple
        # environments in parallel.  Each environment is seeded for
        # reproducibility.
        n_envs = 10
        ...
        # -----------------------------------------------------------------
        # Pretrained branch: load an existing model and skip training
        # -----------------------------------------------------------------
        # Ensure that the provided pretrained_model_path is used as the final
        # model path.  We also set the training history path to None as
        # no training occurs.
        final_model_path = pretrained_model_path
        # Load the provided model using the evaluation environment.  This will
        # restore both policy weights and normalisation statistics.
        best_model = PPO.load(pretrained_model_path, env=eval_env, device=device_used)
        # Remove any existing training history file since we are not training
        if training_history_path.exists():
            training_history_path.unlink()
        # As we do not train in this branch, set the history path to an empty
        # string for clarity when returning results
        training_history_path = ''

    # ---------------------------------------------------------------------
    # Unified evaluation for both branches
    # ---------------------------------------------------------------------
    # Evaluate on the validation set to report final reward.  This step
    # calculates the cumulative reward over the validation period using the
    # deterministic policy from the best (or pretrained) model.
    ...
    # Save monthly weights to CSV.  Include the seed in the filename when
    # provided to avoid overwriting weights from different runs.
    weights_df = pd.DataFrame(weights_history,
                              columns=['Date'] + etf_list)
    if seed is not None:
        weights_csv_path = f'ppo_stage2_weights_seed{run_seed}.csv'
    else:
    ...
    # If best_model_path is a Path object, convert to string; in the pretrained
    # branch best_model_path does not exist on disk, so fall back to
    # final_model_path
    best_path_str: str
    if isinstance(best_model_path, Path) and best_model_path.exists():
        best_path_str = str(best_model_path)
    else:
        best_path_str = final_model_path

    return {
        'final_model_path': final_model_path,
        'best_model_path': best_path_str,
        'weights_csv': weights_csv_path,
        'training_history': str(training_history_path) if training_history_path != '' else '',
