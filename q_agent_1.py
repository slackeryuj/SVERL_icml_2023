...
class Agent:
    """
    Usual Q learning agent, with added functionality for calculating Shapley values.
    """
    ...
    def _default_q_values(self):
        """
        Default function to initialize Q-table entries.
        """
    ...
    def update(self, state, action, new_state, reward, done, info):
        ...
        if done:
            ...
            q_max = self.Q_table[tuple(new_state)].max()
    ...
    def get_policy(self):
        ...
        """
        self.policy = defaultdict(self._default_policy_values)

        for state, q_values in self.Q_table.items():
            # Q values with slightly different values but should be same policy.
            q_values = q_values.round(1)
            self.policy[state] = self._calculate_policy(q_values)

    def _default_policy_values(self):
        """
        Default function to initialize policy table entries.
        """
        return np.full(self.num_actions, 1 / self.num_actions)

    def _calculate_policy(self, q_values):
        """
        Calculate the policy for a given Q-value array.
        """
        policy = (q_values == q_values.max()).astype(float)
        policy /= policy.sum()
        return policy
    ...
    def get_pi_C(self, C, state_dist, states_to_explain):
        ...
        if len(C) == self.state_dim:
            ...
            pi_C = defaultdict(self._default_policy_values)
            temp_pi_C = {}

            for m_state in np.unique(mask_states, axis=0):
                ind = (mask_all_states == m_state).all(axis=1)
                state_dist_cond = state_dist_full[ind] / state_dist_full[
                    ind].sum()  # Conditional limiting state occupancy distribution.

                # pi_C = \sum_{s \in S}{\pi(a|s) * p(s|s_C)}
                temp_pi_C[tuple(m_state)] = (
                        np.array(list(self.policy.values()))[ind] * state_dist_cond[:, None]
                ).sum(axis=0)
